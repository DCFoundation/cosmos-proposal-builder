...
const fetchChainConfig = async (chainName, isTestnet = false) => {
  const url = isTestnet
    ? `${RAW_FILE_REPO_URL}/${GIT_REF}/testnets/${chainName}testnet/chain.json`
    : `${RAW_FILE_REPO_URL}/${GIT_REF}/${chainName}/chain.json`;
  const data = await dw(url);
  ...
  if (!validateChainConfig(config)) {
    console.error(`Chain config for ${chainName} is invalid. Skipping...`);
    return null;
  }
