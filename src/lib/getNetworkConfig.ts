...
const makeAgoricChainConfig = async (netName: NetName): Promise<NetworkConfig> => {
...
const getNetworkConfig = async (chainName: string, netName: string): Promise<NetworkConfig | undefined> => {
  if (chainName === "agoric") {
